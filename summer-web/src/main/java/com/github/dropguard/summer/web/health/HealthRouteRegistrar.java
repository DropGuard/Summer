	...
	public void registerControllers(HttpRouter.Builder builder, BeanContainer context) {
		builder.get("/health/ready", (ctx) -> {
			if (ApplicationState.isShuttingDown()) {
				ctx.json(com.github.dropguard.summer.web.HttpStatus.SERVICE_UNAVAILABLE,
						java.util.Map.of("status", "SHUTTING_DOWN"));
			} else {
				ctx.json(com.github.dropguard.summer.web.HttpStatus.OK, java.util.Map.of("status", "UP"));
			}
		});
