 ...
 *
 * <p>A router maps incoming HTTP requests to their corresponding handlers based on method and path.
 * This interface is <strong>immutable by design</strong> --it exposes only the {@link
 * #route(HttpContext)} method for dispatching requests. Route registration is done via the {@link
 * Builder} inner class.
 *
...
public interface HttpRouter {
     ...
     *
     * <p>Collects route definitions and builds an immutable router on {@link #build()}. The router
     * implementation is determined by the factory function passed to the constructor.
     */
    class Builder {

        /**
         * A route entry containing method, path pattern, and handler. Used to transfer route
         * definitions to router engine factories.
         */
         ...
         *
         * @param routerFactory a function that creates the router implementation from the route
         *     list
         */
         ...
         *
         * <p>The router factory is looked up from the provided {@link RouterRegistry}.
         *
         * @param type the router type to use
         * @param registry the router registry to look up the factory from
        ...
        private Builder route(HttpMethod method, String path, Handler handler) {
            ...
            String fullPath =
                    currentBase != null ? PathUtils.combinePaths(currentBase, path) : path;
        ...
        /**
         * Normalizes Express-style path parameters ({@code :param}) to the canonical {@code
         * {param}} syntax used by all router implementations.
         *
         * <p>Both MapRouter and RadixTreeHttpRouter understand {@code {param}} but only
         * Express-style controllers may declare {@code :param}. This normalization ensures both
        ...
        /**
         * Adds middleware to the current scope. Middleware wraps handlers and adds cross-cutting
         * concerns (authentication, logging, etc.).
         *
         * <p>When used inside a {@link #group}, the middleware only applies to routes within that
         * group. When used at the top level, it applies to all subsequent routes.
         *
        ...
        /**
         * Creates a route group under the given base path. All routes defined within the group
         * callback share the base path prefix and can have shared middleware.
         *
         * <p>Example:
         *
         * <pre>{@code
         ...
         * }</pre>
         *
         * @param basePath the base path prefix for all routes in this group
         * @param groupConfig callback to configure the group
         * @return this builder for chaining
         */
        public Builder group(String basePath, Consumer<Builder> groupConfig) {
            String currentBase = basePathStack.peekLast();
            String newBase =
                    currentBase != null ? PathUtils.combinePaths(currentBase, basePath) : basePath;

            List<Middleware> currentMiddlewares = middlewareStack.peekLast();
            List<Middleware> groupMiddlewares =
                    currentMiddlewares != null
        ...
        /**
         * Mounts a route module. The module callback receives this builder and registers routes on
         * it.
         *
         * <p>This is the primary mechanism for modular route composition:
         *
         * <pre>{@code
         * // Define a module
         ...
         * }</pre>
         *
         * @param module the route module to mount
         * @return this builder for chaining
         */
