 ...
 *
 * <p>This is the missing piece that lets framework-agnostic layers (services, AOP interceptors)
 * read "the current request" without it being passed as a method parameter. It is safe under
 * Summer's execution model because:
 *
 * <ul>
 *   <li>each request runs on its own <em>virtual thread</em> (no pooling / reuse), so there is no
 *       cross-request bleed;
 *   <li>the server entry point ({@code NettyHttpServerHandler}) always clears the holder in a
 *       {@code finally} block at the end of request processing.
 * </ul>
 *
 * <p>Producers: the auth middleware calls {@link #set(RequestContext)} once a request is
