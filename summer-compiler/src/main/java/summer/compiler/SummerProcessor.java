...
public class SummerProcessor extends AbstractProcessor {
	...
	public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
		...
		// --- Collect @RestController beans (string-based, may not be on processor
		// classpath) ---
	...
	private void collectByAnnotationName(String annotationFqn, RoundEnvironment roundEnv) {
		...
		if (annotationType == null)
			return; // annotation not on classpath
	...
	private void generateAotContext() throws IOException {
		...
		JandexDiscovery.BeanCollector collector = new JandexDiscovery.BeanCollector() {
			@Override
			public void collectComponent(TypeElement te) {
				SummerProcessor.this.collectComponent(te);
			}
			@Override
			public void collectConfiguration(TypeElement te) {
				SummerProcessor.this.collectConfiguration(te);
			}
			@Override
			public boolean alreadyCollected(TypeElement te) {
				return SummerProcessor.this.alreadyCollected(te);
			}
		};
		...
		if (restControllerType != null) {
			boolean hasWebController = sorted.stream().anyMatch(
					b -> AnnotationHelper.hasAnnotation(b.typeElement, "summer.web.annotation.RestController"));
			boolean hasExceptionHandler = sorted.stream()
					.anyMatch(b -> ElementFilter.methodsIn(b.typeElement.getEnclosedElements()).stream().anyMatch(
							m -> AnnotationHelper.hasAnnotation(m, "summer.web.annotation.ExceptionHandler")));
	...
	private void analyzeAop() {
		...
		for (BeanDefinition bean : allBeans) {
			...
			if (bean.kind == BeanDefinition.Kind.FACTORY_PRODUCT)
				continue;
			if (bean.interfaces.isEmpty())
				continue;

			boolean hasIntercepted = AotProxyGenerator.beanHasMethodsWithAnnotation(bean.typeElement,
	...
	/**
	 * Checks if a class is an inner class whose enclosing class is not a framework
	 * bean. This filters out test @Configuration inner classes (e.g.
	 * RuntimeBeanTest$TestConfig) that shouldn't be in the AOT context.
	 */
		...
		while (changed) {
			...
			for (BeanDefinition bean : allBeans) {
				...
				for (BeanDefinition other : allBeans) {
					if (other == bean)
						continue;
					if (typeUtils.isAssignable(typeUtils.erasure(other.typeElement.asType()),
	...
	/**
	 * Resolves @Replaces annotations: removes replaced @Configuration classes and
	 * their @Bean factory products from allBeans.
	 */
	private void resolveReplacements() {
		...
		for (BeanDefinition bean : allBeans) {
			if (bean.kind != BeanDefinition.Kind.CONFIGURATION)
				continue;
			if (!AnnotationHelper.hasAnnotation(bean.typeElement, "summer.core.annotation.Replaces"))
				continue;

			List<TypeMirror> targets = AnnotationHelper.getAnnotationClassListValue(bean.typeElement,
			...
			if (replacementMap.containsKey(targetElement)) {
				error("Duplicate @Replaces: both " + replacementMap.get(targetElement).getQualifiedName() + " and "
						+ bean.typeElement.getQualifiedName() + " replace " + targetElement.getQualifiedName(),
						bean.typeElement);
				return;
			}
		...
		allBeans.removeIf(bean -> replacedNames.contains(bean.qualifiedName())
				|| (bean.kind == BeanDefinition.Kind.FACTORY_PRODUCT && bean.configClass != null
