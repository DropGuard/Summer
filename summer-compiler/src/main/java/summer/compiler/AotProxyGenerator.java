 ...
 * Extracted from SummerProcessor to separate proxy code generation from bean collection.
 */
final class AotProxyGenerator {
    ...
    /**
     * Generates an AOT proxy class for the given bean.
     * The proxy implements all interfaces of the target bean and delegates
     * method calls through the interceptor chain without reflection.
     */
    static void generate(BeanDefinition bean, ProcessingEnvironment processingEnv) {
        ...
        for (ProxyMethod pm : uniqueMethods.values()) {
            boolean isIntercepted = shouldInterceptMethod(bean.typeElement, pm.method, bean.interceptors, processingEnv);
            ...
            proxyBuilder.addMethod(buildProxyMethod(pm, isIntercepted, targetFieldName, interfaceFieldName, processingEnv));
        }
    ...
    static boolean beanHasAnnotatedMethods(TypeElement bean, List<TypeMirror> targetAnnotations, ProcessingEnvironment processingEnv) {
    ...
    private static TypeSpec.Builder buildMethodMetadata(TypeElement beanClass, ExecutableElement interfaceMethod,
        ...
        TypeSpec.Builder metadataTarget = TypeSpec.anonymousClassBuilder("")
                ...
                .addMethod(MethodSpec.methodBuilder("getName")
                        ...
                        .addStatement("return $S", interfaceMethod.getSimpleName().toString())
                        .build())
                .addMethod(MethodSpec.methodBuilder("getDeclaringClass")
                        .addAnnotation(Override.class)
                        .addModifiers(Modifier.PUBLIC)
                        .returns(Class.class)
                        .addStatement("return $T.class", ClassName.get(beanClass))
                        .build());
    ...
    private static MethodSpec buildProxyMethod(ProxyMethod pm, boolean isIntercepted,
                                                String targetFieldName, String interfaceFieldName,
                                                ProcessingEnvironment processingEnv) {
    ...
    private static boolean shouldInterceptMethod(TypeElement beanClass, ExecutableElement interfaceMethod,
                                                  List<BeanDefinition> interceptorBeans,
                                                  ProcessingEnvironment processingEnv) {
    ...
    private static TypeElement asTypeElement(TypeMirror typeMirror, ProcessingEnvironment processingEnv) {
        if (processingEnv == null) return null;
