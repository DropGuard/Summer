 ...
 * web routes and exception handlers at compile time.
 * Extracted from SummerProcessor to separate web route generation from bean collection.
 */
final class RouteAdapterGenerator {
    ...
    /**
     * Generates the route adapter if any @RestController or @ExceptionHandler beans exist.
     */
    static void generate(List<BeanDefinition> beans, ProcessingEnvironment processingEnv) {
        ...
        if (restControllerType == null) return;
        ...
        for (BeanDefinition controller : controllers) {
            String basePath = AnnotationHelper.getAnnotationStringValue(controller.typeElement, "summer.web.annotation.RestController", processingEnv);
            ...
            for (ExecutableElement method : ElementFilter.methodsIn(controller.typeElement.getEnclosedElements())) {
                ...
                if (AnnotationHelper.hasAnnotation(method, "summer.web.annotation.Get")) {
                    httpMethod = "GET";
                    methodPath = AnnotationHelper.getAnnotationStringValue(method, "summer.web.annotation.Get", processingEnv);
                } else if (AnnotationHelper.hasAnnotation(method, "summer.web.annotation.Post")) {
                    httpMethod = "POST";
                    methodPath = AnnotationHelper.getAnnotationStringValue(method, "summer.web.annotation.Post", processingEnv);
                } else if (AnnotationHelper.hasAnnotation(method, "summer.web.annotation.Put")) {
                    httpMethod = "PUT";
                    methodPath = AnnotationHelper.getAnnotationStringValue(method, "summer.web.annotation.Put", processingEnv);
                } else if (AnnotationHelper.hasAnnotation(method, "summer.web.annotation.Delete")) {
                    httpMethod = "DELETE";
                    methodPath = AnnotationHelper.getAnnotationStringValue(method, "summer.web.annotation.Delete", processingEnv);
                }
                ...
                if (httpMethod != null) {
                    ...
                    List<TypeMirror> classMiddlewares = AnnotationHelper.getAnnotationClassListValue(controller.typeElement, "summer.web.annotation.Use", processingEnv);
                    List<TypeMirror> methodMiddlewares = AnnotationHelper.getAnnotationClassListValue(method, "summer.web.annotation.Use", processingEnv);
        ...
        for (BeanDefinition component : componentsWithExceptionHandlers) {
            ...
            for (ExecutableElement method : ElementFilter.methodsIn(component.typeElement.getEnclosedElements())) {
                if (AnnotationHelper.hasAnnotation(method, "summer.web.annotation.ExceptionHandler")) {
                    List<TypeMirror> exceptionClasses = AnnotationHelper.getAnnotationClassListValue(method, "summer.web.annotation.ExceptionHandler", processingEnv);
                    CodeBlock args = buildMethodCallArgs(method, types, throwableType, processingEnv);
    ...
    private static CodeBlock buildMethodCallArgs(ExecutableElement method, Types types,
                                                  TypeMirror throwableType, ProcessingEnvironment processingEnv) {
        ...
        for (int i = 0; i < params.size(); i++) {
            ...
            if (paramType.toString().equals("summer.web.WebContext")) {
                ...
                String val = AnnotationHelper.getAnnotationStringValue(param, "summer.web.annotation.PathParam", processingEnv);
