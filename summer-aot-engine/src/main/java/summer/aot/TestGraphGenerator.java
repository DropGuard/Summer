...
/**
 * Generates a minimal AOT {@code BeanContainer} for a test class annotated
 * with {@code @SummerTest(engine = AOT, value = {...})}.
 *
 * <p>
 * Computes the transitive dependency closure from entry beans using pure
 * Jandex BFS (mirrors the runtime {@code transitiveExpand}), then
 * generates a {@code TestGraph_<testClassName>} class containing only the
 * beans in that closure.
 * </p>
 */
public final class TestGraphGenerator {
    ...
    private static final DotName CONFIGURATION_PROPERTIES = DotName.createSimple(
    ...
    // ---- transitive closure (pure Jandex BFS, mirrors RuntimeApplicationContext.transitiveExpand) ----

    /**
     * Computes the transitive dependency closure of the given entry beans,
     * walking constructor parameters, {@code @Bean} method return types,
     * and {@code @Replaces} targets through the Jandex index.
     *
     * @param entryNames qualified class names of the entry beans
     * @param index      Jandex index of all available classes
    ...
    public static Set<String> transitiveClosure(Set<String> entryNames, IndexView index) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (ci.isInterface() || ci.isAbstract()) {
                ...
                for (ClassInfo impl : index.getKnownDirectImplementors(ci.name())) {
            ...
            for (Type paramType : ctor.parameterTypes()) {
                ...
                // Handle List<T>
                if ("java.util.List".equals(paramName)
                        && paramType.kind() == Type.Kind.PARAMETERIZED_TYPE) {
        ...
        log.debug("[Summer] transitiveClosure: {} seeds → {} closure beans",
                entryNames.size(), closure.size());
        return closure;
    }

    private static void expandImplementors(String typeName, IndexView index,
            Set<String> closure, Deque<String> queue) {
        ...
        for (ClassInfo impl : index.getKnownDirectImplementors(dotName)) {
    ...
    /**
     * Generates a {@code TestGraph_<testClassName>} source file containing only
     * the beans identified by the scoped discovery.
     *
     * @param testClassName fully-qualified name of the test class
     * @param sortedBeans   topologically-sorted beans in the closure
     * @param outputDir     directory to write the generated {@code .java} file
     */
    public void generate(String testClassName, List<BeanDefinition> sortedBeans,
            IndexView index, java.io.File outputDir) throws IOException {

        String safeName = "TestGraph_" + testClassName.replace('.', '_').replace('$', '_');
        ...
        TypeSpec contextClass = TypeSpec.classBuilder(safeName)
                ...
                .addJavadoc("Auto-generated AOT TestGraph. Do not edit manually!\n")
                .addJavadoc("Generated by summer-maven-plugin for test class: " + testClassName + "\n")
                .addMethod(buildCreateMethod(sortedBeans, index))
                .build();
        ...
        log.debug("[Summer] Generated TestGraph for {} ({} beans)", testClassName, sortedBeans.size());
    }

    private MethodSpec buildCreateMethod(List<BeanDefinition> sortedBeans, IndexView index) {
        MethodSpec.Builder method = MethodSpec.methodBuilder("create")
                .addModifiers(javax.lang.model.element.Modifier.PUBLIC, javax.lang.model.element.Modifier.STATIC)
                .returns(CN_BEAN_CONTAINER)
        ...
        method.addStatement("$T.setDefaultValueResolver(($T) Class.forName($S).getField($S).get(null))",
                CN_CONFIG_BINDER, CN_DEFAULT_VALUE_RESOLVER,
        ...
        wm.emitRowMapperRegistry(method, index, closureNames);
