...
public final class WireMethodGenerator {
	...
	private static final ClassName JDBC_TEMPLATE = ClassName.get("com.github.dropguard.summer.data.jdbc",
			"JdbcTemplate");
	...
	void emitRowMapperRegistrations(MethodSpec.Builder wire, IndexView index, java.util.Set<String> activeClassNames,
		...
		List<com.github.dropguard.summer.data.jdbc.RowModelMeta> metas = com.github.dropguard.summer.data.jdbc.RowMapperFactory
				.scanJandex(index);
		...
		for (var meta : metas) {
			...
			wire.addStatement("$T<$T> $N = ($N, $N) -> {",
					ClassName.get("com.github.dropguard.summer.data.jdbc", "RowMapper"), modelClass, mapperVar, "rs",
					"rowNum");
			for (var field : meta.fields()) {
				String colName = com.github.dropguard.summer.data.jdbc.RowMapperFactory.camelToSnake(field.name());
	...
	private void emitComponentInstantiation(MethodSpec.Builder wire, BeanDefinition bean, ClassName beanClass,
		...
		if (bean.needsProxy() && bean.interfaceNames.isEmpty()) {
			throw new com.github.dropguard.summer.aop.SummerAopException(
					...
					bean.qualifiedName + " is annotated with AOP bindings but implements no interfaces. "
		...
		if (bean.needsProxy()) {
			...
			wire.addStatement("$T<$T> $N = new $T<>()", ClassName.get(List.class),
					ClassName.get("com.github.dropguard.summer.aop", "MethodInterceptor"), interceptorsListVar,
	...
	private void emitConfigPropertiesInstantiation(MethodSpec.Builder wire, ConfigPropertiesBean bean,
		...
		ClassName configBinder = ClassName.get("com.github.dropguard.summer.core.config", "ConfigBinder");
	...
	private static CodeBlock buildDefaultsLiteral(Map<String, String> defaults, Map<String, String> typeNames) {
		...
		for (Map.Entry<String, String> e : defaults.entrySet()) {
			...
			cb.add("$S, $T.convert($S, $T.class)", e.getKey(),
					ClassName.get("com.github.dropguard.summer.core.config", "TypeConverter"), e.getValue(),
