 ...
 * {@link com.github.dropguard.summer.aop.InterceptedMethod} — a zero-
...
public final class AotProxyGenerator {
	...
	private static final ClassName PROXY_CHAIN = ClassName.get("com.github.dropguard.summer.aop",
			"ProxyInterceptorChain");
	private static final ClassName INTERCEPTED_METHOD = ClassName.get("com.github.dropguard.summer.aop",
			"InterceptedMethod");
	...
	public void generate(List<BeanDefinition> beans, IndexView index, java.io.File outputDir) throws IOException {
		...
		for (ClassInfo ci : index.getKnownClasses()) {
			if (ci.isAnnotation()
					&& ci.hasAnnotation(DotName.createSimple("com.github.dropguard.summer.aop.InterceptorBinding"))) {
	...
	private void generateProxy(BeanDefinition bean, IndexView index, java.io.File outputDir,
		...
		ClassName interceptorType = ClassName.get("com.github.dropguard.summer.aop", "MethodInterceptor");
