 ...
 *
 * <p>For each bean with interceptors, generates a {@code $$AotProxy} class that implements the same
 * interfaces and delegates to the target bean through the interceptor chain. Uses {@link
...
public final class AotProxyGenerator {
    ...
    private static final ClassName PROXY_CHAIN =
            ClassName.get("com.github.dropguard.summer.aop", "ProxyInterceptorChain");
    private static final ClassName INTERCEPTED_METHOD =
            ClassName.get("com.github.dropguard.summer.aop", "InterceptedMethod");
     ...
     * @param index Jandex index for looking up interface method signatures
            ...
            if (ci.isAnnotation()
                    && ci.hasAnnotation(
                            DotName.createSimple(
                                    "com.github.dropguard.summer.aop.InterceptorBinding"))) {
    ...
    private void generateProxy(
        ...
        ParameterizedTypeName interceptorList =
                ParameterizedTypeName.get(ClassName.get(List.class), interceptorType);
        ...
        JavaFile proxyFile =
                JavaFile.builder(packageName, proxyBuilder.build()).indent("    ").build();
    ...
    private MethodSpec buildProxyMethod(MethodInfo method) {
        ...
        for (MethodParameterInfo param : method.parameters()) {
            ...
            if (name == null) name = "arg" + paramIdx;
            paramNames.add(name);
        ...
        body.addStatement(
                "var chain = new $T(target, $N, args, interceptors, $L)",
                PROXY_CHAIN,
                metaName,
                lambda);
    ...
    private static TypeName toTypeName(org.jboss.jandex.Type type) {
        if (type.name() == null) return ClassName.get(Object.class);
        String name = type.name().toString();
        if (name.equals("void")) return TypeName.VOID;
        // Primitive mapping is shared with the rest of the AOT codegen via TypeReads.
        if (PrimitiveTypes.isPrimitive(name)) return TypeReads.typeName(name);
