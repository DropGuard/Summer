...
public final class RouteAdapterGenerator {
	...
	public void generate(List<BeanDefinition> beans, java.io.File outputDir) throws IOException {
		...
		JavaFile.builder("com.github.dropguard.summer.core.aot", routeRegistrar).build().writeTo(outputDir);
	}
		...
		for (RouteInfo.ParamInfo param : route.params) {
			if (param.binding == RouteInfo.ParamBinding.PATH) {
				...
				body.add(
						...
						ClassName.get("com.github.dropguard.summer.web", "HttpParameterResolverChain"),
						ClassName.get("com.github.dropguard.summer.web", "RouteInfoHandlerParam"),
						ClassName.bestGuess(param.type), param.bindingName,
						ClassName.get("com.github.dropguard.summer.core.bean.RouteInfo", "ParamBinding"),
						param.validated);
			}
		}
