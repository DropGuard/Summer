...
/**
 * Verifies the code-generation type artifacts in {@link TypeReads}: type-name
 * to {@link com.palantir.javapoet.TypeName} mapping, HTTP parse transforms, and
 * the JDBC read expression for {@code @RowModel} fields.
 */
class TypeReadsTest {

	@Test
	void jdbcReadEmitsGetObjectWithClassLiteral() {
		assertEquals("rs.getObject(\"created_at\", java.time.LocalDateTime.class)",
				TypeReads.jdbcRead("created_at", "java.time.LocalDateTime").toString());
		assertEquals("rs.getObject(\"id\", java.lang.Long.class)",
				TypeReads.jdbcRead("id", "java.lang.Long").toString());
		assertEquals("rs.getObject(\"uuid\", java.util.UUID.class)",
				TypeReads.jdbcRead("uuid", "java.util.UUID").toString());
		assertEquals("rs.getObject(\"amount\", java.math.BigDecimal.class)",
	...
	void jdbcReadRejectsUnsupportedTypeThroughSharedContract() {
		...
		IllegalStateException ex = org.junit.jupiter.api.Assertions.assertThrows(IllegalStateException.class,
				() -> TypeReads.jdbcRead("nested", "com.example.Address"));
		org.junit.jupiter.api.Assertions.assertTrue(ex.getMessage().contains("Unsupported @RowModel field type"));
	}
	...
	void httpParseWrapsNumericTypes() {
		...
		assertEquals("java.lang.Integer.parseInt(ctx.request().pathParam(\"id\"))", intRead.toString());

		CodeBlock strRead = TypeReads.httpParse("java.lang.String", "ctx.request().queryParam", "q");
