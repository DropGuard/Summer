...
public final class RowMapperGenerator {
	...
	public void generate(IndexView index, File outputDir) throws IOException {
	...
	private void generateMapper(ClassInfo ci, File outputDir) throws IOException {
		...
		for (RecordComponentInfo comp : components) {
			com.palantir.javapoet.TypeName compType = TypeNameUtil.fromString(comp.type().name().toString());
