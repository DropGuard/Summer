 ...
 * </p>
 * <ul>
 * <li>{@code DiEngine} (in {@code com.github.dropguard.summer.core}) — the
 * single engine loader; it reflectively loads the compiled container/engine
 * class. Both the production path and the test-time AOT compiler funnel through
 * it, so reflective loading of generated classes lives in exactly one
 * place.</li>
 * <li>{@code com.github.dropguard.summer.runtime} — the runtime engine
 * discovers and instantiates beans via reflection; this is its defining
 * mechanism.</li>
 * <li>{@code com.github.dropguard.summer.test} — test infrastructure
 * reflectively loads the optional AOT engine and Mockito to break compile-time
 * dependency cycles.</li>
 * <li>{@code com.github.dropguard.summer.data.jdbc} — loads user-declared
 * {@code @RowModel} record classes so it can build their {@code RowMapper}s;
 * loading domain models is a data-module responsibility, not a cross-module
 * reflection. The AOT path emits these mappings at compile time (zero
 * reflection); only the runtime engine reflects here, symmetrically with how it
 * loads {@code @Component}s.</li>
 * </ul>
 *
 * <p>
...
class ReflectionConfinementTest {
	...
	void reflectionConfinedToRuntime() {
        ...
        ArchRule rule = noClasses()
                .that().resideOutsideOfPackage("..com.github.dropguard.summer.runtime..")
                .and().resideOutsideOfPackage("..com.github.dropguard.summer.test..")
                .and().resideOutsideOfPackage("..com.github.dropguard.summer.data.jdbc..")
