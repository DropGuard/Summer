...
/**
 * Pins the serialization architecture: in lazy mode the template and its codec must share a single
 * {@link ObjectMapper} instance, and a value must round-trip through that one mapper. Two
 * independent mappers would drift and break round-trips the moment either is customized.
 */
public class RedisTemplateSerializationContractTest {
    ...
    void codecEncodesAndDecodesThroughSingleMapper() {
        ...
        Event decoded =
                codec.mapper()
                        .convertValue(
                                codec.mapper().convertValue(original, Object.class), Event.class);
