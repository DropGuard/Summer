 ...
 *
 * <ul>
 *   <li><b>Lazy (preferred)</b> — bound to a {@link RedisClient} and a {@link JsonRedisCodec}. The
 *       network connection is opened on the first command, never at construction time, so a context
 *       can be assembled in an environment without a reachable Redis (unit tests, offline builds).
 *       Serialization is performed by the codec, which owns the single {@link ObjectMapper} shared
 *       with this template.
 *   <li><b>Eager</b> — wrapped around an already-resolved {@link RedisCommands} handle (e.g.
 *       injected directly, or for tests). The connection and its codec are owned by the caller;
 *       this template is a thin, serialization-agnostic facade.
 * </ul>
 *
 * <h3>Serialization contract</h3>
 *
 * Every stored value is JSON. A missing key reads back as {@code null}; a {@code null} value is
 * never written — remove a key with {@link #delete(String)} instead.
 *
 * <h3>Lifecycle</h3>
 *
 * In lazy mode the template is {@link AutoCloseable}; when the enclosing framework container
 * closes, the underlying connection and client are released. In eager mode the caller owns the
 * resources and this template's {@link #close()} is a no-op.
 *
 * @see JsonRedisCodec
...
public class SummerRedisTemplate implements AutoCloseable {
    ...
    /**
     * Wraps an already-resolved commands handle. Serialization is the caller's responsibility (the
     * connection's codec decides it); this template uses a standalone mapper only for in-memory
     * type conversion.
     *
     * @param commands the Redis commands handle
     */
    ...
    /**
     * Wraps an already-resolved commands handle with an explicit mapper used for in-memory type
     * conversion.
     *
     * @param commands the Redis commands handle
     * @param objectMapper mapper for converting decoded values to target types
     */
    ...
    /**
     * Binds to a {@link RedisClient} and {@link JsonRedisCodec}. The connection is opened lazily on
     * the first command, so construction never requires a reachable Redis server. The template
     * shares the codec's {@link ObjectMapper} as the single source of truth for (de)serialization.
     *
     ...
     * @param key the Redis key
     * @param value the value to store (must not be {@code null})
     * @param ttl the time-to-live; honored at millisecond precision
     * @throws IllegalArgumentException if {@code value} is {@code null}
     */
    ...
    /**
     * Advanced operations escape hatch. Returns the underlying sync commands for callers needing
     * commands beyond the typed single-key API (e.g. sorted-set fan-out). The returned handle
     * shares this template's codec, so values are serialized with the same JSON contract.
     *
    ...
    /**
     * Executes a Lua script atomically on the server. Exposed for operations that need
     * compare-and-act semantics (e.g. a flash-sale decrement) beyond the typed single-key API.
     *
     * @param <T> the script return type
     * @param script the Lua source
     * @param type the expected output type
     * @param keys keys referenced by {@code KEYS[n]}
    ...
    private static void requireNonNullValue(Object value) {
        if (value == null) {
            throw new IllegalArgumentException(
                    "Cannot store null in Redis; remove a key with delete(key) instead.");
        }
    }

    /**
     * Releases the underlying connection and client in lazy mode. In eager mode the caller owns the
     * resources, so this is a no-op. Safe to call multiple times.
     */
    @Override
    public void close() {
        if (closed) {
