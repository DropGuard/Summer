...
public class NettyHttpServerHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
	...
	public NettyHttpServerHandler(NettyHttpServer server, ServerConfig config, WebServerDependencies deps) {
		this.server = server;
		this.config = config;
		this.deps = deps;
	}
			...
			try {
				...
				if (deps.exceptionRegistry() != null) {
					...
					if (customHandler != null) {
						try {
							c.request().setAttribute(com.github.dropguard.summer.web.RequestAttributes.LAST_EXCEPTION,
									e);
