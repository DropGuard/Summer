...
public class NettyServerRunner implements ApplicationRunner {
    ...
    /**
     * Type key under which {@code SummerApplication} registers the ordered list of middleware
     * classes declared via {@code apply(...)}. A plain JDK {@code List} type is used as the bean
     * key so no dedicated carrier type is needed.
     */
    @SuppressWarnings("unchecked")
    private static final Class<List<Class<? extends com.github.dropguard.summer.web.Middleware>>>
            APPLY_LIST_TYPE =
                    (Class<List<Class<? extends com.github.dropguard.summer.web.Middleware>>>)
                            (Class<?>) List.class;
    ...
    public void run(BeanContainer context) throws Exception {
        ...
        List<com.github.dropguard.summer.web.Middleware> globalMiddlewares =
                collectGlobalMiddlewares(context);
     ...
     *
     * <ol>
     *   <li>middleware classes declared via {@code SummerApplication.apply(...)} — explicit, in
     *       declaration order (registered as a plain {@code List<Class<? extends Middleware>>}
     *       bean);
     *   <li>{@code @GlobalMiddleware}-annotated {@code Middleware} beans, in container registration
     *       order.
     * </ol>
     *
     * Explicit declarations take precedence because the caller ordered them by hand; annotated
     * beans fill in everything else. A middleware bean is only added once even if both mechanisms
     * reference it.
     */
    private List<com.github.dropguard.summer.web.Middleware> collectGlobalMiddlewares(
        ...
        try {
            for (Class<? extends com.github.dropguard.summer.web.Middleware> c :
                    context.getBean(APPLY_LIST_TYPE)) {
                if (added.add(c)) {
        ...
        for (com.github.dropguard.summer.web.Middleware m :
                context.getBeans(com.github.dropguard.summer.web.Middleware.class)) {
            if (m.getClass().isAnnotationPresent(GlobalMiddleware.class)
                    && added.add(m.getClass())) {
                result.add(m);
            }
        }
        return result;
    ...
    private ExceptionRegistry buildExceptionRegistry(BeanContainer context) {
        ...
        for (ExceptionHandlerRegistrar registrar :
                context.getBeans(ExceptionHandlerRegistrar.class)) {
    ...
    private WsRouter buildWsRouter(BeanContainer context) {
        ...
        for (com.github.dropguard.summer.web.WsRouteProvider provider :
                context.getBeans(com.github.dropguard.summer.web.WsRouteProvider.class)) {
