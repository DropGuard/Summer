...
/**
 * Utility class for authentication-related operations. Extracts common authentication logic from
 * controllers.
 */
public final class AuthUtils {
    ...
    /**
     * Extracts the JWT token from the Authorization header. Expected format: "Token <jwt>"
     *
    ...
    /**
     * Returns the authenticated user's ID, or {@code null} when no token is present (anonymous
     * access). If a token is supplied it must be valid — expired or malformed tokens throw a 401
