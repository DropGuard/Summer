...
/**
 * Phase 0 of the DI build-chain redesign: confirm (or refute) the AOT
 * {@code build(MockedBean[])} dual-method generation path on a CLEAN seeded
 * universe.
 *
 * <p>
 * The AOT engine keeps two generated entry points: the production-only
 * {@code build(Object...)} (single method, emitted when mocks are null) and the
 * test-only typed {@code build(MockedBean[])} (emitted when mocks are
 * non-null). The typed path is the one Quarkus-aligned test containers and
 * {@code @Mock} exercise — but it was never covered by any existing test that
 * uses the pre-generated single-method context from the maven plugin. So this
 * test forces it: {@link Testing#buildNarrowAot} passes a NON-null (empty)
 * mocks array, which makes {@code AotContextGenerator} emit
 * {@code build(MockedBean[])} instead of {@code build(Object...)}.
 * </p>
 *
 * <p>
 ...
 * symptom was discovery-stage noise, not a codegen defect.
 * </p>
 */
class AotDualMethodBehaviorTest {
	...
	/**
	 * The non-empty mocks branch must still generate, compile, and load a working
	 * container. This exercises {@code build(MockedBean[])} with a real (non-null)
	 * mock array — proving the typed channel is not just syntactically valid but
	 * end-to-end runnable — while the empty-mocks test above proves the graph wires
	 * through. Mock *replacement* semantics (the stub standing in for the real
	 * bean) is a separate concern verified by {@code MockBehaviorTest} through the
	 * full {@code @Mock} pipeline, not the narrow direct-AOT entry used here.
	 */
	@Test
	void dualMethodPathWithMockArrayGeneratesCompilesAndLoads() {
		...
		assertNotNull(container, "AOT build(MockedBean[]) path with a non-empty mock array must return a container");
		// The non-mocked beans must still resolve — the mock only targets ServiceC.
		assertNotNull(container.getBean(ServiceA.class), "ServiceA must still wire under the mock-array path");
		assertNotNull(container.getBean(ServiceB.class), "ServiceB must still wire under the mock-array path");
	}
}
