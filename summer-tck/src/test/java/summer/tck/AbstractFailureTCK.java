...
public abstract class AbstractFailureTCK extends AbstractTCK {
	...
	/**
	 * Assert that {@link #triggerFailure()} throws an exception
	 * whose message contains one of the expected strings.
	 */
	protected void assertFailureContains(String... expectedMessages) {
		...
		boolean matches = Arrays.stream(expectedMessages)
				.anyMatch(msg -> exception.getMessage().contains(msg));

		assertTrue(matches,
				"Expected failure message to contain one of " + Arrays.toString(expectedMessages)
