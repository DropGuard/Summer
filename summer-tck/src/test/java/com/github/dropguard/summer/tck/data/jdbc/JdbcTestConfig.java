 ...
 *
 * <p>
 * With {@link JdbcTemplate} present as a bean,
 * {@code ReflectiveRowMapperRegistrar} (Runtime engine) auto-registers every
 * {@code @RowModel} it discovers through the unified test universe (the
 * deployment's discovery index combines {@code jandex.idx} with the running
 * test class's {@code test-classes} directory, indexed on demand). The
 * test-tree {@code @RowModel} {@code User} is therefore discovered and mapped
 * on BOTH engines — the real dual-engine path this TCK must verify. No manual
 * {@code registerMapper} here: doing so would bypass
 * {@code ReflectiveRowMapperRegistrar} and hide exactly the gap this TCK exists
 * to catch.
 * </p>
 *
 * <p>
