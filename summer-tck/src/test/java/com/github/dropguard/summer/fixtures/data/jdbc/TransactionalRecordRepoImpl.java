 ...
 *
 * <p>{@link #insertThenFail(Long)} demonstrates the contract the rollback TCK verifies: it writes a
 * row inside a transaction and then throws, so the framework must roll the insert back. The
 * interface (implemented here) is what makes the JDK proxy — and therefore the transaction boundary
 * — possible in Summer's AOP model.
 */
@Component
