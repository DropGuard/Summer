 ...
 *
 * <p>Two {@code @Bean} methods return the same type ({@code MethodReplacesBean}); exactly one
 * carries {@code @Replaces(MethodReplacesBean.class)}. After evaluation the original must be gone
 * and the replacer must survive. This guards against {@code Discovery} leaking a class-level
 * {@code @Replaces} onto the {@code @Bean} product (which once caused both products to be removed).
 */
class MethodReplacesDiscoveryTest {
    ...
    void methodLevelReplacesKeepsReplacerAndDropsOriginal() throws Exception {
        ...
        List<BeanDefinition> survivors =
                beans.stream()
                        .filter(
                                b ->
                                        "com.github.dropguard.summer.fixtures.di.MethodReplacesBean"
                                                .equals(b.qualifiedName))
                        .toList();
        assertEquals(1, survivors.size(), "exactly one MethodReplacesBean bean should remain");
        assertNotNull(
                survivors.get(0).methodLevelReplaces,
                "the survivor must be the @Replaces-annotated replacer");
    }
}
