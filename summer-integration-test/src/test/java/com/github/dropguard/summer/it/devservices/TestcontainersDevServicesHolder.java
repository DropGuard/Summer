 ...
 * — for the whole test run, and hands out one shared {@link DataSource} pool
 * and one Redis URI. That eliminates the per-{@code @SummerTest} pool
 * proliferation that previously exhausted Postgres' {@code max_connections}.
 * </p>
 *
 * <p>
 * This is an <b>internal</b> convenience: only framework-owned integration
 * tests (this module, and any other internal test module) use it. Demos and
 * external users must NOT — they use the public path (start Testcontainers
 * directly and set {@code com.github.dropguard.summer.test.datasource.url} /
 * {@code com.github.dropguard.summer.redis.uri}), exactly like
 * {@code RedisIntegrationIT} does.
 * </p>
 */
public class TestcontainersDevServicesHolder implements DevServicesHolder {
	...
	public synchronized ConnectionDescriptor start(Map<String, String> requestedEnvironment) {
		...
		postgres = new PostgreSQLContainer<>(DockerImageName.parse("postgres:15")).withDatabaseName(dbName)
	...
	/**
	 * Redis URI for the shared container (redis://host:port), for
	 * {@code com.github.dropguard.summer.redis.uri}.
	 */
