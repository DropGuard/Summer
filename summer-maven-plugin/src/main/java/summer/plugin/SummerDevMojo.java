...
@org.apache.maven.plugins.annotations.Execute(phase = org.apache.maven.plugins.annotations.LifecyclePhase.COMPILE)
public class SummerDevMojo extends AbstractMojo {

	@Parameter(defaultValue = "${project}", readonly = true, required = true)
	private MavenProject project;

	@Parameter(property = "summer.dev.port", defaultValue = "8080")
	private int port;
	@Parameter(property = "summer.mainClass", required = false)
	...
	public void execute() throws MojoExecutionException {
		...
		try {
			...
			if (mainClass == null) {
				...
				if (mainClass == null) {
					throw new MojoExecutionException(
							"Could not auto-detect main class. Please specify <summer.mainClass> in pom.xml properties.");
				}
			...
			DirectoryWatcher watcher = new DirectoryWatcher(getLog(), new File(project.getBasedir(), "src/main/java"));
			watcher.start(changedFile -> {
				appManager.kill(); // EAGER KILL: break the pipe
				proxy.changedFiles.add(changedFile);
				proxy.isDirty = true; // LAZY COMPILE: proxy will pick this up on next request
			});
		...
		try (java.io.FileInputStream fis = new java.io.FileInputStream(idxFile)) {
			...
			for (org.jboss.jandex.ClassInfo clazz : index.getKnownClasses()) {
				for (org.jboss.jandex.MethodInfo method : clazz.methods()) {
					...
					if (method.name().equals("main") && (method.flags() & ACC_STATIC) != 0
