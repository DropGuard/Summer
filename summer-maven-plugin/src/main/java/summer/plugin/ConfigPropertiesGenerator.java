...
final class ConfigPropertiesGenerator {
    ...
    private void generateProvider(ClassInfo ci, AnnotationInstance ann, File outputDir) throws IOException {
        ...
        MethodSpec.Builder provideMethod = MethodSpec.methodBuilder("provide")
                .addAnnotation(Override.class)
                .addModifiers(Modifier.PUBLIC)
                .returns(configClass);

        if (prefix.isEmpty()) {
            provideMethod.addStatement("return $T.bind($S, $T.class)",
                    CONFIGURATION_BINDER, "application.yml", configClass);
        } else {
            provideMethod.addStatement("return $T.bind($S, $T.class, $S)",
                    CONFIGURATION_BINDER, "application.yml", configClass, prefix);
        }
        ...
        TypeSpec providerClass = TypeSpec.classBuilder(providerClassName)
                ...
                .addSuperinterface(ParameterizedTypeName.get(PROVIDER, configClass))
                .addMethod(provideMethod.build())
                .build();

        JavaFile javaFile = JavaFile.builder(packageName, providerClass)
                .indent("    ")
                .build();
