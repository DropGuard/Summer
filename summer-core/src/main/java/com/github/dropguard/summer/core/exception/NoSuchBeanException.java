 ...
 *
 * <p>Carries the structured context of the failure (what was missing, who needed it, and the
 * registered candidates) as fields — not just a hand-built message — so logs and tests consume the
 * data uniformly. The human-readable message is assembled in exactly one place (the structured
 * constructor); callers pass the pieces rather than building strings themselves.
 */
public class NoSuchBeanException extends SummerException {
    ...
    private static String render(
        ...
        sb.append("No bean found for dependency type: ")
                .append(missingType)
                .append(" required by ")
        ...
        if (!nearMisses.isEmpty()) {
            sb.append("\n  candidates with matching simple name '")
                    .append(simpleName(missingType))
                    .append("': ")
