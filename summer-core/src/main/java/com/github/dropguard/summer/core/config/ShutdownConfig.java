 ...
 *
 * <p>{@code timeoutMs} bounds how long in-flight requests are allowed to drain before a server's
 * resources are released. There is no hard traffic-isolation sleep: an instance signals shutdown
 * via {@code ApplicationState.beginShutdown()} (readiness probe returns 503), letting the load
 * balancer stop routing before the server stops accepting — so the drain window is driven by LB
