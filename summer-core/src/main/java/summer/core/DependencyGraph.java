...
/**
 * Dependency graph manager that handles dependency resolution and cycle detection.
 */
public class DependencyGraph {
    ...
    private Set<Class<?>> getDependencies(Class<?> clazz) {
        ...
        return Arrays.stream(constructor.getParameterTypes())
                .filter(paramType -> graph.containsKey(paramType))
    ...
    private Constructor<?> findConstructor(Class<?> clazz) {
        ...
        if (constructors.length != 1) {
            throw new SummerException("Component " + clazz.getName()
    ...
    /**
     * Performs topological sort on the dependency graph to determine instantiation order.
     */
