 ...
 *
 * <p>
 * A profile lets a test run the <em>same</em> bean universe under different
 ...
 *
 * <p>
 * Override keys are dotted paths in the original YAML key form (e.g.
 * {@code server.port}); the framework normalizes them to match record
 * components and applies them through the shared {@code ConfigBinder}
 * chokepoint, so both DI engines (Runtime and AOT) see identical overrides.
 * </p>
 */
public interface TestProfileSpec {
	...
	/**
	 * Configuration-property overrides applied for this profile. Keys are dotted
	 * paths in the original YAML key form; values are the raw bound values.
	 *
