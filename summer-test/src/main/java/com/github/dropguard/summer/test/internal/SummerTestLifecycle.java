...
public final class SummerTestLifecycle {
	...
	private java.util.Map<String, Object> profileOverrides(Class<?> testClass) {
		...
		try {
			com.github.dropguard.summer.test.profile.TestProfileSpec spec = ann.value().getDeclaredConstructor()
					.newInstance();
	...
	private DevServicesHolder createHolder() {
		try {
			Class<?> holderClass = Class
					.forName("com.github.dropguard.summer.test.devservices.TestcontainersDevServicesHolder");
