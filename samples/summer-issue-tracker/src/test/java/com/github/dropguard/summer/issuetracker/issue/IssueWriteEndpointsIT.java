...
/**
 * Fine-grained write authorization on the issue PUT endpoints — the rules that {@link
 * com.github.dropguard.summer.issuetracker.security.ProjectAuthorization#assertOwns} enforces.
 * {@link com.github.dropguard.summer.issuetracker.security.RbacIT} covers create/delete; here we
 * hit update / status / assign / priority directly and assert that a plain member may only mutate
 * issues assigned to or reported by them, a viewer may not write at all, and an outsider is
 * forbidden entirely.
 */
class IssueWriteEndpointsIT extends AbstractIssueTrackerIT {
    ...
    void managerCanMutateAnyIssue() throws Exception {
        ...
        assertEquals(
                200,
                put("/api/issues/" + issueId + "/status", "{\"status\":\"IN_PROGRESS\"}", o.token())
                        .statusCode());
        assertEquals(
                200,
                put("/api/issues/" + issueId + "/priority", "{\"priority\":\"HIGH\"}", o.token())
                        .statusCode());
    }
    ...
    void memberCanMutateOnlyOwnIssues() throws Exception {
        ...
        assertEquals(
                ...
                post(
                                ...
                                "{\"userId\":%d,\"role\":\"MEMBER\"}".formatted(m.userId()),
                                o.token())
                        .statusCode());
    ...
    void viewerCannotWrite() throws Exception {
        ...
        assertEquals(
                ...
                post(
                                "/api/projects/" + projectId + "/members",
                                "{\"userId\":%d,\"role\":\"VIEWER\"}".formatted(v.userId()),
                                o.token())
                        .statusCode());
        long issueId = createIssue(projectId, o.token(), null);
        ...
        assertEquals(
                403,
                put("/api/issues/" + issueId + "/status", "{\"status\":\"IN_PROGRESS\"}", v.token())
                        .statusCode(),
        ...
        assertEquals(
                ...
                put(
                                ...
                                "{\"assigneeId\":%d}".formatted(v.userId()),
                                v.token())
                        .statusCode());
    }
