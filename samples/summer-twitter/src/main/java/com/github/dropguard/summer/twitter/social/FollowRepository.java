...
public class FollowRepository {
    ...
    public List<Follow> findFollowers(Long followingId, Long cursor, int limit) {
        if (cursor == null) {
            String sql =
                    "SELECT id, follower_id, following_id, created_at FROM follows WHERE"
                            + " following_id = ? ORDER BY id DESC LIMIT ?";
            return jdbcTemplate.queryForList(sql, Follow.class, followingId, limit);
        } else {
            String sql =
                    "SELECT id, follower_id, following_id, created_at FROM follows WHERE"
                            + " following_id = ? AND id < ? ORDER BY id DESC LIMIT ?";
    ...
    public List<Follow> findFollowing(Long followerId, Long cursor, int limit) {
        if (cursor == null) {
            String sql =
                    "SELECT id, follower_id, following_id, created_at FROM follows WHERE"
                            + " follower_id = ? ORDER BY id DESC LIMIT ?";
            return jdbcTemplate.queryForList(sql, Follow.class, followerId, limit);
        } else {
            String sql =
                    "SELECT id, follower_id, following_id, created_at FROM follows WHERE"
                            + " follower_id = ? AND id < ? ORDER BY id DESC LIMIT ?";
    ...
    public List<Long> findInfluencerFollowing(Long followerId, int followerCountThreshold) {
        String sql =
                "SELECT f.following_id FROM follows f JOIN users u ON f.following_id = u.id WHERE"
                        + " f.follower_id = ? AND u.follower_count >= ?";
