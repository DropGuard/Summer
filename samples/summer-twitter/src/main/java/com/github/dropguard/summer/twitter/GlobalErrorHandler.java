 ...
 *
 * <p>Business errors extend {@link BusinessException} and already carry the HTTP status + a stable
 * error code, so one handler renders them all. Everything else falls through to the catch-all,
 * which returns a fixed 500 body and leaks no internal detail (the real cause is logged by the
 * framework).
 *
 * <p>Controllers therefore never need hand-written {@code try/catch}: they call services that throw
 * typed exceptions, and this handler translates them.
 */
@Component
public class GlobalErrorHandler {
    ...
    public void handleValidation(HttpContext ctx, ValidationException e) {
        ctx.json(HttpStatus.BAD_REQUEST, new ErrorResponse("VALIDATION_ERROR", e.errors().get(0)));
    }
