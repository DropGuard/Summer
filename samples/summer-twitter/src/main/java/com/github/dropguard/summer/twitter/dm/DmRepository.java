...
public class DmRepository {
    ...
    public DirectMessage insertMessage(DirectMessage msg) {
        ...
        jdbcTemplate.update(
                "INSERT INTO direct_messages (id, sender_id, receiver_id, text, read_at,"
                        + " created_at) VALUES (?, ?, ?, ?, ?, ?)",
                toInsert.id(),
                toInsert.senderId(),
                toInsert.receiverId(),
    ...
    public void upsertConversation(Long userId1, Long userId2, OffsetDateTime lastMessageAt) {
        ...
        int updated =
                jdbcTemplate.update(
                        "UPDATE conversations SET last_message_at = ? WHERE user_one_id = ? AND"
                                + " user_two_id = ?",
                        now,
                        userOneId,
                        userTwoId);
        ...
        if (updated == 0) {
            ...
            jdbcTemplate.update(
                    "INSERT INTO conversations (id, user_one_id, user_two_id, last_message_at,"
    ...
    public List<Conversation> findConversations(Long userId) {
        return jdbcTemplate.queryForList(
                "SELECT * FROM conversations WHERE user_one_id = ? OR user_two_id = ? ORDER BY"
    ...
    public List<DirectMessage> findMessages(Long userId1, Long userId2, Long cursor, int limit) {
        ...
        return jdbcTemplate.queryForList(
                "SELECT * FROM direct_messages WHERE ((sender_id = ? AND receiver_id = ?) OR"
                    + " (sender_id = ? AND receiver_id = ?)) AND id < ? ORDER BY id DESC LIMIT ?",
                DirectMessage.class,
                userId1,
                userId2,
                userId2,
