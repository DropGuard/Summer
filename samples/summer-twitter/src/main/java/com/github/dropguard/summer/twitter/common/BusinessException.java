 ...
 *
 * <p>Every business error carries the {@link HttpStatus} it should produce, so a single
 * {@code @ExceptionHandler(BusinessException.class)} in {@code GlobalErrorHandler} can render it
 * without per-case branching. This is the contract new users should copy: throw a typed exception
 * at the service layer, let it propagate, and let the global handler translate it to a response —
 * never hand-roll {@code try/catch} in controllers.
 */
