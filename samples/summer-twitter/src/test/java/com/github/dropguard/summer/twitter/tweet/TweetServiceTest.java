 ...
 *
 * <p>Behavioral unit test (not {@code @SummerTest}): the service's direct collaborators ({@code
 * TweetRepository}, {@code UserRepository}, {@code TimelineService}) are mocked and {@link
 * SnowflakeIdGenerator} stays real. No framework container, no {@code DataSource} — the JDBC layer
...
class TweetServiceTest {
    ...
    void createTweetReturnsNewId() {
        when(mockUserRepo.findById(1L))
                .thenReturn(
                        Optional.of(
                                new User(1L, "alice", "Alice", "a@x.com", "h", "bio", 0, 0, null)));
        ...
        when(mockUserRepo.findById(1L))
                .thenReturn(
                        Optional.of(
                                new User(1L, "alice", "Alice", "a@x.com", "h", "bio", 0, 0, null)));
        User alice =
                new User(100L, "alice", "Alice", "alice@test.com", "hash", "bio", null, null, null);
    ...
    void deleteTweetThrowsNotFoundWhenMissing() {
        assertThrows(ResourceNotFoundException.class, () -> tweetService.deleteTweet(999L, 1L));
    }
