 ...
 *
 * <p>Behavioral unit test (not {@code @SummerTest}): the only external seam here is Redis, mocked
 * at the {@link SummerRedisTemplate} boundary (a pinned {@link RedisCommands} instance so chained
 * {@code zadd}/{@code zrevrange} calls are verifiable). The application-internal collaborators
 * ({@code TweetRepository}, {@code FollowRepository}) are also mocked. No framework container and
 * no {@code DataSource} — the Redis/JDBC wiring is covered by the integration test against a real
 * Postgres.
 *
 * <p>{@code fanOut} spawns a virtual thread for the follower fan-out when the author has fewer than
 * 5000 followers, which makes assertions on the fan-out non-deterministic from the test thread.
 * Those cases are left to integration coverage; here we exercise the synchronous author-timeline
 * write (follower count ≥ 5000, no virtual thread) and the fully synchronous {@code getTimeline}
...
class TimelineServiceTest {
    ...
    void setUp() {
        ...
        timelineService =
                new TimelineService(mockRedis, mockTweetRepo, mockFollowRepo, mockEventPublisher);
    ...
    void getTimelineReturnsEmptyWhenNoIds() {
        ...
        when(mockFollowRepo.findInfluencerFollowing(7L, TimelineService.INFLUENCER_THRESHOLD))
                .thenReturn(List.of());
    ...
    void getTimelineMergesAndScoresTweets() {
        ...
        Tweet t10 =
                new Tweet(
                        10L, 1L, "a", "POST", null, 5, 0, 0, OffsetDateTime.now().minusMinutes(10));
        Tweet t11 =
                new Tweet(
                        11L, 2L, "b", "POST", null, 50, 1, 2, OffsetDateTime.now().minusMinutes(1));
