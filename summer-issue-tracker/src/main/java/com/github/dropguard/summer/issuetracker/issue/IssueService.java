 ...
 * which require the proxied bean to implement an interface, so {@code @Transactional} and
 * {@code @RequireRole} can be applied.
 *
 * <p>Note what is <em>absent</em>: there is no {@code actorId} parameter on any method. The current
 * user is resolved inside the service (for fine-grained ownership checks and the audit trail's
 * actor) and in {@link com.github.dropguard.summer.issuetracker.security.RbacInterceptor} (for the
 * coarse-grained gate) from {@link com.github.dropguard.summer.web.RequestContextHolder} —
 * published by the framework's auth middleware. This is the corrected design: the Gin-style "thread
 * the user id through every call" was replaced by a framework-level request context.
 */
@RequireRole
