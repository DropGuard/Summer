...
public class IssueController {
    ...
    public void byKey(HttpContext ctx, @PathParam("key") String key) {
        ...
        if (dash <= 0) {
            throw summer.issuetracker.common.BusinessException.badRequest("Invalid issue key");
        }
        String projectKey = key.substring(0, dash);
        long actorId = SecurityContext.currentUserId();
        User actor = userRepository.findById(actorId)
                .orElseThrow(() -> summer.issuetracker.common.BusinessException.unauthorized("Unknown actor"));
        var project = projectRepository.findByKey(actor.orgId(), projectKey)
                .orElseThrow(() -> summer.issuetracker.common.BusinessException.notFound("Project"));
