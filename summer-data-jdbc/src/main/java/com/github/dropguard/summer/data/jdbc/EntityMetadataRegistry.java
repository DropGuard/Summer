...
/**
 * In-memory registry of {@link EntityMetadata}, populated once during container assembly from the
 * same {@code @RowModel} scan that registers RowMappers.
 *
 * <p>QueryBuilder reads exclusively from this registry — it never re-scans the Jandex index, so
 * there is exactly one discovery pass per deployment. The registry is keyed by entity class; {@link
 * #register(RowModelMeta)} is called by {@code ReflectiveRowMapperRegistrar} for every discovered
 * model.
 */
@Configuration
public final class EntityMetadataRegistry {
    ...
    public void register(RowModelMeta meta) {
        ...
        byClassName.put(
                meta.modelClassName(),
                new EntityMetadata(meta.tableName(), columns, meta.fields()));
    }
    ...
    public EntityMetadata get(Class<?> entityClass) {
        ...
        if (meta == null) {
            throw new IllegalArgumentException(
                    "Not a registered @RowModel entity: "
                            + entityClass.getName()
                            + ". Ensure it is annotated with @RowModel(table=...) and discovered by"
                            + " the container.");
        }
        return meta;
    }
