 ...
 * {@code @Transactional} and
 * {@link com.github.dropguard.summer.data.jdbc.JdbcTemplate} is handled inside
 * {@code JdbcTemplate} itself (it reuses the active transaction's connection
 * when one exists on the current thread), so users never wrap their
