 ...
 *
 * <p>This is the write-side counterpart of the reflective {@code RowMapper}: both recover entity
 * state through runtime reflection because {@code summer-aot-engine} does not yet generate
 * {@code @RowModel} accessors. Keeping the reflection here (rather than inside the builders) means
 * a future AOT accessor can replace this single class without changing the public {@code
 * QueryTemplate} API.
 */
final class EntityAccessor {
    ...
    /**
     * Returns an ordered map from snake_case column name to the entity's field value, following the
        ...
        try {
            ...
            throw new IllegalStateException(
                    "Cannot read field '"
                            ...
                            + "' from "
                            + type.getName()
                            + ". @RowModel entities must expose a record component or getter for"
                            + " every mapped field.",
                    e);
        }
    }
