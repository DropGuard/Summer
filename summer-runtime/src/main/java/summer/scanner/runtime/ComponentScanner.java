...
public class ComponentScanner {
	...
	/**
	 * Discovers @Component-annotated classes (including meta-annotations like
	 * @Configuration, @RestController, @GlobalMiddleware).
	 * Framework beans come from pre-built Jandex indexes;
	 * user beans are discovered by indexing the given packages.
	 */
	...
	/**
	 * Indexes user packages at runtime. Handles both file: and jar: classpath entries.
	 */
	...
	private void tryIndexJar(Indexer indexer, URL url) {
		...
		if (!urlStr.startsWith("jar:file:")) return;
	...
	/**
	 * Loads pre-built {@code META-INF/jandex.idx} files from all framework JARs on the classpath.
	 */
	 ...
	 * recursive meta-annotation chain). Covers @Configuration, @RestController,
	 * @GlobalMiddleware, and any future annotations meta-annotated with @Component.
	 */
	...
	private boolean hasComponentMetaAnnotation(ClassInfo classInfo, IndexView index, Set<DotName> visited) {
		...
		if (!visited.add(name)) return false; // cycle guard

		if (classInfo.hasAnnotation(COMPONENT)) return true;
	 ...
	 * other configurations, validates no duplicate replacements, and removes replaced
	 * classes from the component set.
	 *
	 * @return the set of replaced configuration classes (so callers can also exclude their @Bean producers)
	 */
		...
		for (Class<?> clazz : new ArrayList<>(componentClasses)) {
			...
			if (!componentClasses.contains(target)) {
				throw new SummerException(ErrorCode.BEAN_CREATION_FAILED,
					"@Replaces target " + target.getName() + " (on " + clazz.getName()
