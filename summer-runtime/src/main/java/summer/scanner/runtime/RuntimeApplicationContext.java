...
public class RuntimeApplicationContext implements ApplicationContext {
    ...
    /** Tracks AutoCloseable singletons in instantiation order for reverse-order shutdown. */
     ...
     * Internal use only — prefer classpath scanning via {@link #scan()}.
     */
    ...
    public void initializeBeans() {
        ...
        // Index @Bean factory methods from @Configuration classes (before instantiation)
    ...
    private void evaluateConditions() {
        ...
        while (changed) {
            ...
            for (Class<?> clazz : new ArrayList<>(components)) {
                summer.core.annotation.ConditionalOnBean cond =
                        clazz.getAnnotation(summer.core.annotation.ConditionalOnBean.class);
                ...
                if (!satisfied) {
                    satisfied = beanProducers.containsKey(requiredType)
                            || beanProducers.entrySet().stream()
    ...
    /**
     * Finds an @Bean producer whose return type is assignable to the requested type.
     */
    ...
    private Object invokeBeanProducer(Method producer, Class<?> producedType) {
        try {
            ...
            if (configBean == null) {
                throw new BeanCreationException(
                    "Configuration bean not instantiated: " + configClass.getName());
            }
            Object[] args = Arrays.stream(producer.getParameterTypes())
                    .map(this::getBean)
                    .toArray();
    ...
    public <T> T getBean(Class<T> type) {
        ...
        // Assignability match in produced singletons (e.g. DataSource <- HikariDataSource)
    ...
    /**
     * Closes all AutoCloseable singletons in reverse instantiation order.
     * Errors during individual close() calls are logged but do not abort the shutdown sequence.
     */
    @Override
    public void destroy() {
