 ...
 *
 * <p>The interception decision (which interface method is wrapped, and by which interceptors) is
 * computed at assembly time by {@link RuntimeAopProcessor} from the bean's pre-enriched binding
 * data ({@code BeanDefinition}), not by reflection on the hot path. This factory is therefore a
 * pure lookup table: it receives a {@link ProxyMethodSpec} per interface method and, at dispatch,
...
public class ProxyFactory {
    ...
    /**
     * Per-method proxy plan: the interceptors to run for a method, plus the binding annotation
     * types that method carries (exposed to interceptors via {@link
     * com.github.dropguard.summer.aop.InterceptedMethod#isAnnotationPresent}). Computed once at
     * assembly time, never on the hot path.
     */
    public record ProxyMethodSpec(
            List<MethodInterceptor> interceptors, Set<Class<? extends Annotation>> bindings) {
    ...
    private static class ProxyInvocationHandler implements InvocationHandler {
        ...
        public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
            ...
            try {
                var chain =
                        new ProxyInterceptorChain(
                                target,
                                new com.github.dropguard.summer.aop.InterceptedMethod(
                                        method.getName(), spec.bindings()),
                                args,
                                spec.interceptors(),
