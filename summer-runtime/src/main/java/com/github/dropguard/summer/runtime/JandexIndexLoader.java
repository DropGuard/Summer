 ...
 *
 * <p>This is the single source of truth for reading the <b>production</b> index. It enforces the
 * Quarkus-style <b>disjoint-pipeline</b> model: the production pipeline and the test pipeline are
 * entirely separate concerns.
 *
 * <ul>
 *   <li><b>Production / AOT pipeline</b> — every {@code META-INF/jandex.idx} on the classpath (one
 *       per module, written at compile time). Read <em>only</em> by this loader's {@link
 *       #productionIndex()}. A test class is never part of this result, so a production container
 *       (runtime startup or AOT code generation) can never instantiate a test bean. This is the
 *       boundary that keeps {@code SummerMojo}-generated AOT containers free of test classes.
 *   <li><b>Test pipeline</b> — has <em>no</em> pre-baked index file. A whole-universe
 *       {@code @SummerTest} container indexes the current test class's {@code test-classes}
 *       directory on demand (see {@code TestClassIndexer}), exactly like Quarkus'
 *       {@code @QuarkusTest} re-indexing its own test-classes as an additional archive. A narrow
 *       {@code @SummerTest(classes=...)} container reads the named {@code .class} bytes directly
 *       ({@code NarrowIndexBuilder}). Because the test pipeline is built from {@code .class} bytes
 *       — never from a merged {@code jandex-test.idx} — there is no full classpath sweep and no
 *       exclude list; a negative (sad-path) fixture, which lives in its own module, is simply not
...
public final class JandexIndexLoader {
    ...
    /**
     * The production pipeline: every {@code META-INF/jandex.idx} on the classpath. Test-class
     * indexes are <b>never</b> part of this result — so a production container (runtime startup or
     * AOT generation) can never instantiate a test bean. This is the boundary that keeps {@code
     * SummerMojo}-generated AOT containers free of test classes.
     *
    ...
    public static LoadedIndex productionIndex() {
        ...
        if (archiveIndexes.isEmpty()) {
            throw new ConfigurationException(
                    ErrorCode.CONFIG_MISSING_INDEX,
                    "No "
                            + PRODUCTION_INDEX
                            + " found on classpath. Ensure jandex-maven-plugin is configured for"
                            + " modules that ship beans.");
        }
    ...
    /**
     * Holds a loaded index together with the per-archive attribution maps, so the caller ({@code
     * BeanDeployment}) can build a production deployment without re-scanning the classpath.
     */
    ...
    /**
     * Loads every classpath resource with the given name, attributing each class to the module
     * derived from its origin URL. Multiple copies of the same module on the classpath (e.g. {@code
     * target/classes} and the installed jar) are merged into one archive index rather than
     * last-wins, so a stale jar index cannot drop classes that the freshly compiled output still
     * has.
     */
    private static void loadIndexResource(
            String resourceName,
            Map<String, String> classToArchive,
            Map<String, IndexView> archiveIndexes) {
        ...
        try {
            Enumeration<URL> urls =
                    JandexIndexLoader.class.getClassLoader().getResources(resourceName);
     ...
     *
     * <p>For jar files, uses the artifact name (stripped of version): {@code
     * summer-data-jdbc-0.1.0.jar} → {@code summer-data-jdbc}.
     *
     * <p>For directory-based indexes (local build output), uses the project directory name: {@code
     * .../summer-core/target/classes/META-INF/jandex.idx} → {@code summer-core}.
     */
