 ...
 *
 * <p>This class reproduces that model for Summer: {@link #indexTestClasses(Class)} indexes only the
 * {@code test-classes} directory resolved from the given test class's code source — nothing more.
 * No {@code jandex-test.idx}, no full classpath sweep, no allow/deny list. A negative fixture is
 * reachable only through the narrow {@code @SummerTest(classes=...)} path (which indexes the named
