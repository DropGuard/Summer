...
/**
 * Comprehensive QA hands-on integration test for the Summer framework.
 * Tests real runtime behavior through the public API.
 *
...
public class QaHandsOnIntegrationTest {
    ...
    class DiEngineCore {
        ...
        void circularDependencyDetected() {
            assertThrows(CircularDependencyException.class,
                    () -> createContext(CircularA.class, CircularB.class));
        }
        ...
        void providerPattern() {
            ...
            // The StringProvider implements Provider<String>, so String should be resolvable
    ...
    class AopProxyTests {
        ...
        @Test
        @DisplayName("P1-3: ProxyFactory creates working proxy with @Intercepted method")
        ...
        @Test
        @DisplayName("P1-4: ProxyFactory passes through non-@Intercepted methods unchanged")
        void proxyFactoryPassesThroughNonIntercepted() {
            ...
            InterceptedService proxy = ProxyFactory.createProxy(target, List.of(interceptor));
            // nonInterceptedGreet has no @Intercepted, so it should pass through
        ...
        void proxyFactoryRejectsNonInterface() {
            ...
            assertThrows(SummerAopException.class,
                    () -> ProxyFactory.createProxy(target, List.of()));
        }

        @Test
        @DisplayName("P1-6: Multiple interceptors execute in order on @Intercepted method")
    ...
    class WebRouterTests {
        ...
        void routeConflictDetection() {
            ...
            assertThrows(RouteConflictException.class,
                    () -> router.get("/users/{name}", ctx -> "conflict"));
        }
    ...
    class EdgeCaseTests {
        ...
        void threadSafeSingletonAccess() throws Exception {
            ...
            for (int i = 1; i < threadCount; i++) {
                assertSame(first, results.get(i),
                        "Thread " + i + " got a different singleton instance");
            }
            ctx.destroy();
        }
        ...
        void bodyParsingRequiresRecord() {
            Request req = new Request("POST", "/test", null, "application/json",
    ...
    // Interface with @Intercepted methods for AOP testing
