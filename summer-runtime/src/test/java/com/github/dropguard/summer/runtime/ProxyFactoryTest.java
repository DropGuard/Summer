...
import com.github.dropguard.summer.fixtures.di.runtime.TestIntercepted;
 ...
 *
 * <p>The factory is a pure dispatch table: the caller (RuntimeAopProcessor) decides, per interface
 * method, which interceptors wrap it and which binding annotation types the method carries. These
 * tests therefore supply an explicit {@code Map<Method, ProxyMethodSpec>} rather than expecting the
 * factory to rediscover bindings by reflection.
 */
class ProxyFactoryTest {

    private static ProxyFactory.ProxyMethodSpec spec(
            MethodInterceptor interceptor, Class<? extends Annotation>... bindings) {
    ...
    private static ProxyFactory.ProxyMethodSpec spec(
            List<MethodInterceptor> interceptors, Class<? extends Annotation>... bindings) {
        Set<Class<? extends Annotation>> set = new java.util.HashSet<>();
        java.util.Collections.addAll(set, bindings);
    ...
    void shouldHandleMultipleInterceptors() {
        ...
        TestService proxy =
                ProxyFactory.createProxy(
                        ...
                        Map.of(
                                sayHello,
                                spec(List.of(interceptor1, interceptor2), TestIntercepted.class)));
        String result = proxy.sayHello();
    ...
    void shouldHonorInterfaceOnlyBindingPlan() {
        ...
        InterfaceAnnotatedService proxy =
                ProxyFactory.createProxy(
                        ...
                        Map.of(
                                sayHello,
                                spec(interceptor, TestIntercepted.class),
                                greet,
                                spec(interceptor, TestIntercepted.class)));
